...
def setup_environment(args):
    os.environ['ION_API_CACHE_ENABLED'] = ('1' if args.cache else '0')
    os.environ['ION_API_CACHE_REDIS_ENDPOINT'] = args.cache_redis_endpoint
    os.environ['ION_API_CACHE_REDIS_PORT'] = str(args.cache_redis_port)

    os.environ['ION_API_LOGS_LEVEL'] = args.logs_level
    os.environ['ION_API_LOGS_JSONIFY'] = ('1' if args.logs_jsonify else '0')

    os.environ['ION_API_ROOT_PATH'] = args.root
    os.environ['ION_API_GET_METHODS_ENABLED'] = ('1' if args.get_methods else '0')
    os.environ['ION_API_JSON_RPC_ENABLED'] = ('1' if args.json_rpc else '0')
    
    os.environ['ION_API_TONLIB_LITESERVER_CONFIG'] = args.liteserver_config
    os.environ['ION_API_TONLIB_KEYSTORE'] = args.tonlib_keystore
    os.environ['ION_API_TONLIB_PARALLEL_REQUESTS_PER_LITESERVER'] = str(args.parallel_requests_per_liteserver)
    if args.cdll_path is not None:
        os.environ['ION_API_TONLIB_CDLL_PATH'] = args.cdll_path
    return


def main():
    parser = argparse.ArgumentParser('ion-http-api')
    ...
    tonlib_args.add_argument('--liteserver-config', type=str, default='https://cdn.ice.io/mainnet/global.config.json', help='Liteserver config JSON path')
    tonlib_args.add_argument('--tonlib-keystore', type=str, default='./ion_keystore/', help='Keystore path for tonlibjson')
    ...
    if args.version:
        os.system('pip3 show ion-http-api')
        return
